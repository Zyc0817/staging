                    ...
                    <el-form :model="ruleForm" ref="ruleForm" size="medium" class="demo-ruleForm">
                            ...
                            <el-form-item label="使用部门：" style="width:380px;">
                                <div class="userDepartmentBorder">
                                    <div class="tagWrap" v-for="(i,k) in departmentList" :key="k">
                                        <div class="tag">{{i.value|changeType}}</div> 
                            ...
                            <div class="deviceManagerTitle">管理员<span></span>：</div>
                                <div class="userDepartmentBorder">
                                    <div class="tagWrap" v-for="(item,key) in equipmentList" :key="key">
                                        <div class="tag">{{item.value|changeType}}</div>   
                        ...
                        <div class="equipmentIP">
                            <el-form-item label="设备位置：" style="width:380px;">
                                <el-input v-model="ruleForm.location" placeholder="请输入" style="margin-left:10px;"></el-input>
                            </el-form-item>
                        </div>
                        <div class="equipmentFigure">
                            <el-form-item label="图片：" style="width:380px;">
                                <div class="equipmentFigureBorder">
                                    <img :src= photoUrl+i.filepath+i.filedate  alt="" class="existingImage" v-for="(i,k) in photoList" :key="k">
                                    <div v-if="photoList.length<2">
                                        <el-upload
                                            :action= "dialogVisibleUrl"
                                            list-type="picture-card"
                                            :limit="photoList.length==1?1:2" 
                                ...
                                <span v-if="photoList.length<2">最多上传2张，最大不超过8M (选填)</span>
                            </el-form-item>
                        </div>
                    </el-form>
                </div>
...
export default {
	...
	methods: {
        dialogInit1() {
            let arr = this.departmentList.map(i=>{
                    return i.key
                })
                this.checkedNodes = this.departmentList.map(i=>{
                    return {
                        id: i.key,
                        name: i.value
                    }
        ...
        tagInit1() {
            let arr = []
                arr = this.equipmentList.map(i=>i.key)
                this.checkedNodes1 = this.equipmentList.map(i=>{
                    return {
                        title: i.value
                    }
                })
        ...
        sure() { 
            this.departmentList=this.checkedNodes.map(i=>{
                return {
                    key: i.id,
                    value: i.name
                }
        ...
        init() {
            this.$axios({
                ...
                this.value = _.get(res,'data.content.businessType','')
                this.ruleForm.serial = _.get(res,'data.content.serialNo','')
                this.ruleForm.equipment = _.get(res,'data.content.ip','')
                this.departmentList = _.get(res,'data.content.departments',[])
                this.equipmentList = _.get(res,'data.content.managers',[])
                this.ruleForm.location = _.get(res,'data.content.location','')
        ...
        confirm() {
            ...
            this.$axios({
                method: 'put',
                url: this.$urls.machinedEdit,
                data: { 
                    id: this.$store.state.systemSettings.id,
                    businessType: this.value,
                    serialNo: this.ruleForm.serial,
                    ip: this.ruleForm.equipment,
                    departments: this.departmentList,
                    managers: this.equipmentList,
                    location: this.ruleForm.location,
                    imageOne: this.srcOne?this.srcOne:arr[0],
                    imageTwo: this.srcTwo?this.srcTwo:arr[1]
                }
            }).then(res=>{
        ...
        ofCourse() {
            ...
            this.equipmentList = Object.assign(this.equipmentList,arr)
            this.dialogVisibleTwo = false
        },
		cancel() {
			this.show = false
			this._timeOut = setTimeout(()=>{
				this.$store.dispatch('editEquipment')
			}, 200)
        },
        deleteDepartment(k) {
